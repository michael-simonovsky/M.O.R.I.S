//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!PICK ONE TO CHOOSE THE NODE !!!!!!!!!!!!!!!!!!!!!!!!
    // Slampher Smart Light Bulb Socket
#include "SlamphrMstrBth33.h"  //OTA deployed date:
...
void mqttSubs(char* topic, byte* payload, unsigned int length) {	// receive and handle MQTT messages
	...
	Serial.println();
#endif
	if (strlen(topic) == 5) {						// correct topic length ?   01234  temp6 
		DID = (topic[4]-'0')*10 + topic[3]-'0';	// extract device ID from MQTT topic
	...
	if (length == 0) {error = 2;}					// no payload sent
      ...
      if (readAction) {
		...
		if (error !=0) {							// send error message
				sprintf(buff_topic, "tem06", nodeId);
...
#endif
			if (client.connect(clientName,"tem06",0,true,clientName)) { 
...
void wifiOTAcheck(){
  ...
  Serial.println(OTAname);
	...
	if (send0) {									// send uptime
		sprintf(buff_topic, "temo06", nodeId);
	...
	if (send3) {									// send software version000000000
...
//===============================================================================================
  ...
  ArduinoOTA.handle();
	...
	client.loop();
	  sprintf(buff_topic, "temp1", nodeId);
    sprintf(buff_msg, "sup",  nodeId, clientName);
    pubMQTT(buff_topic, buff_msg);
